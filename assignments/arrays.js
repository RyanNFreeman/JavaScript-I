...
let carModels = [];
for (let i = 0; i < inventory.length; i++) {
    let carAlph = inventory[i].car_model;
    carModels.push(carAlph);
...
let oldCars =[];
inventory.forEach(carDate => {
    if (carDate.car_year < 2000) oldCars.push(carDate.car_year);
});
console.log(oldCars); 
...
const expensive = inventory.filter(make => {
    if (make.car_make === "BMW" || make.car_make === "Audi") {
        return true;
    }
});

BMWAndAudi.push(expensive);
